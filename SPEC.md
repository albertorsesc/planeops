...
| `secrets` | list | no | `[]` | Items: `{ref: secret://<backend>/<name>, injected_as: file:<path>#KEY}`. Validated at registry load; `env:NAME` is rejected as not yet supported, and unknown item keys are rejected like any other typo |
