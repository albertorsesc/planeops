...
from engine.adapters.launchd import ADAPTER, LaunchdAdapter, RunResult, parse_launchctl_list
...
def _entry(entry_id, lifecycle):
    return entry_from_dict(
        {"id": entry_id, "adapter": "launchd", "domain": "service", "lifecycle": lifecycle, "intent": "i"}
    )
...
def _obs(label, *, loaded, pid=None, plist_path="/tmp/x.plist"):
    ...
    return Observed("launchd", label, {"loaded": loaded, "pid": pid, "plist_path": plist_path})
...
def test_observe_reports_loaded_and_running(tmp_path, fake_platform):
    ...
    out = {o.native_id: o for o in LaunchdAdapter(run=_list_run).observe(_ctx(fake_platform(tmp_path)))}
...
def test_plan_retired_but_loaded_proposes_bootout():
    changes = ADAPTER.plan(_entry("launchd/svc", "retired"), _obs("svc", loaded=True, pid=99))
    assert len(changes) == 1
    assert changes[0].kind == "remove"
    assert changes[0].action == {"op": "bootout", "label": "svc", "plist_path": "/tmp/x.plist", "delete_plist": False}
...
def test_plan_conformant_states_propose_nothing():
    ...
    assert ADAPTER.plan(_entry("launchd/svc", "retired"), _obs("svc", loaded=False)) == []
...
def test_execute_bootout_calls_launchctl_and_reports_ok():
    ...
    change = Change("launchd/svc", "remove", "d", {"op": "bootout", "label": "svc", "delete_plist": False})
...
def test_execute_purge_deletes_plist(tmp_path):
    ...
    change = Change("launchd/svc", "remove", "d", {"op": "bootout", "label": "svc", "plist_path": str(plist), "delete_plist": True})
...
def test_execute_bootstrap_calls_launchctl():
    rec = RecordingRun(code=0)
    change = Change("launchd/svc", "configure", "d", {"op": "bootstrap", "label": "svc", "plist_path": "/p.plist"})
    res = LaunchdAdapter(run=rec).execute(change, _ctx(None))
    assert res.ok
