...
def _ctx(entries, *, prior=None, attest=False, interactive=False, now=None):
    return Ctx(
        platform=object(),
        host="testhost",
...
def test_attest_records_now_and_not_stale():
    ...
    out = ADAPTER.observe(_ctx([_entry()], attest=True, interactive=True, now=now))
    assert out[0].facts["attested_at"] == now.isoformat()
    assert out[0].facts["stale"] is False


def test_attest_records_even_when_piped():
    # `--attest` is explicit intent; a non-TTY run must still record it.
    now = datetime(2026, 7, 22, 12, 0, 0)
    out = ADAPTER.observe(_ctx([_entry()], attest=True, interactive=False, now=now))
    assert out[0].facts["attested_at"] == now.isoformat()
    assert out[0].facts["stale"] is False
